...
import (
	...
	"github.com/JokingLove/market-services/common/httputil"
	"github.com/JokingLove/market-services/config"
	"github.com/JokingLove/market-services/database"
	"github.com/JokingLove/market-services/services/rest/routes"
...
func (a *API) initRouter(conf config.ServerConfig, cfg *config.Config) {
	...
	svc := service.NewHandleService(v, a.db.MarkerPrice, a.db.OfficialCoinRate)
...
func (a *API) Stop(ctx context.Context) error {
	var result error
	if a.apiSrv != nil {
		if err := a.apiSrv.Stop(ctx); err != nil {
